...
export class AuthService extends BaseService {
    ...
    async saveNewUserData(req: Request, res: Response, next: NextFunction, dto: RegisterUserDTO) {
        ...
        const referralLink = dto.baseUrl + "?referral=" + referralId
        let register: IRegisterModel = req.app.locals.register({ firstName, lastName, email: email.toLowerCase(), password: hashedPassword, phoneNumber: phoneNumber, uuid: uuid(), referralLink: referralLink, referralId: referralId });
        let responseObj = null
            ...
            if (result) {
                ...
                await token.save().then(output => {
                    if (output) {
                        ...
                        responseObj = new BasicResponse(Status.SUCCESS, { token: tokenData.token, id: result._id, msg: "A verification email has been sent to " + result.email + ".", referralId: result.referralId });
                        return next();
                    } else {
    ...
    public async loginUser(req: Request, res: Response, next: NextFunction) {
        ...
        await req.app.locals.register.findOne({ email: dto.username.toLowerCase() }).then(async result => {
            if (result) {
